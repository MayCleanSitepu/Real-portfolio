...
export default function CardGalaxy() {
  ...
  const cardPositions = useMemo(() => {
    ...
    for (let i = 0; i < numCards; i++) {
      ...
      const layerRadius = 15 + (i % 3) * 4 // 3 different radius layers: 12, 16, 20
  ...
  return (
    <>
      ...
      <Sphere args={[12, 32, 32]} position={[0, 0, 0]}>
        <meshStandardMaterial color="#ff6c6cff" transparent opacity={0.05} wireframe />
      </Sphere>

      <Sphere args={[16, 32, 32]} position={[0, 0, 0]}>
        <meshStandardMaterial color="#f0ff6cff" transparent opacity={0.03} wireframe />
      </Sphere>

      <Sphere args={[20, 32, 32]} position={[0, 0, 0]}>
        <meshStandardMaterial color="#b4ff6c" transparent opacity={0.02} wireframe />
      </Sphere>
